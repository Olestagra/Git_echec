	...
	while(!Terminated){
	   if(Form1->AttendreCoupDistant())
			{   information="Attente du coup distant";
				...
				Synchronize(&MiseAJourInterfacePrincipale);
				//if(monEchiquier.DemiCoup()==1){Timer1->Enabled=true;}
			}
			else
			{   information="À vous de jouer avec les ";
				...
				if(Form1->mode==2)
				{	modeServeur.EnvoyerUnMessage(message,strlen(message)+1); //0 final
				}
				if(Form1->mode==3)
